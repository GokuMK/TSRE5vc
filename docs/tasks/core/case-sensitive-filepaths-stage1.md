...
Its counts predate the post-stage-A fixes below; earlier warning counts and
apparent coverage must be interpreted accordingly.
...
- Empty optional filename semantics, the ENV water-map lookup base, ownerless
  include contexts, and unknown binary SD extensions still need attention in B.
  CVF EngineData classification, ENV RAW scalar selection, and binary SD root
  support are implemented in the follow-up below. Other unclassified formats
  and implicit companion families remain outside certified coverage.
...
## Post-stage-A planner and binary SD follow-up (2026-09-15)

This remains a read-only planner. The changes do not implement game-root apply,
reference-edit execution, or rollback. Neither supplied installation was modified,
and the large-root evaluation has not been rerun; the saved 161-error total above
is historical, not a result for this revision.

- **Binary SD:** the independent scanner reads the descriptor root filename and
  recognizes the supported ESD metadata blocks. The shape document reader now
  selects a filename-before-children binary root layout for the `JINX0t` envelope;
  the existing writer can save it. Unknown binary SD fields remain discovery
  errors instead of silently receiving complete coverage.
- **Correction to the four reported binary SD cases:** direct read-only inspection
  of `GLOBAL/SHAPES/DB/DB23_a1tPnt10dLft.sd`, `DB23_a1tPnt10dLftMnl.sd`,
  `DB23_a1tPnt10dRgt.sd`, and `DB23_a1tPnt10dRgtMnl.sd` found compressed `JINX0s`
  shape geometry, including shape-header and geometry blocks. These are not SD
  descriptors. They retain errors with an explicit envelope-kind mismatch
  explanation; the tool does not reinterpret or replace them automatically.
- **CVF EngineData:** restrict vehicle lookup to consist/activity document
  contexts. Cab metadata such as `EngineData ( SP45 / SU45 )` no longer becomes
  the invented external path `//SP45.eng`.
- **CVF includes:** graphics use the owning CVF's resource directory, including
  when the scalar lives in a shared include. Nested include lookup still starts
  from the include file's directory. Undiscovered owners remain unresolved;
  an arbitrary nearby texture or vehicle is not used to invent an owner.
- **ENV water maps:** select the single RAW filename scalar with or without a
  leading numeric parameter (`256 Wsib-W.raw` occurs in MSTS USA1 ENV files).
  That number is not a filename. The lookup base
  remains unestablished. Absence from the entire inventory permits a missing-content
  warning; a basename present in an unrelated directory does not resolve the
  reference and still requires a lookup rule. Ambiguous RAW fields retain errors.
- **Empty quoted filename fields:** emit `empty-reference`, separate from both
  missing-content warnings and external/invalid-path errors. The explanation says
  optional-field semantics have not been established. Empty stem references do
  not acquire synthetic suffix-only targets such as `.con`. This does not claim
  that every optional empty field is invalid; field-specific exemptions can follow.

Binary SD validation uses independently constructed bytes, rather than a fixture
generated by the writer being tested. Tests cover plain and compressed input,
actual save/reload in temporary files, exact unchanged decompressed bytes, and a
different-length filename edit with every metadata byte retained. Compression
streams need not be identical after recompression; decompressed payloads must be.
Tests also cover text conversion, truncated input, unknown binary SD fields, and
S geometry incorrectly named `.sd`. The reported DB assets are format-mismatch
examples, not evidence of successful round trips on genuine third-party binary SD.

Validation: the Windows main executable and both standalone test targets build
with Qt 6.10.1 / MinGW 13.1. The shape document suite passes 249 checks; the
`content_case` CTest suite passes, including SD scoped resolution, CVF metadata
and include bases, ENV scalar selection, and distinct empty-reference diagnostics. A main-EXE
CLI smoke test on temporary copies of the four DB files reports exactly four
document-kind mismatch errors, as expected, and proposes no operations. The copies
and outputs are under ignored `build/content-case-evaluation/sd-kind-check-*`;
all reads from the supplied roots were read-only.

The existing text SD filename/metadata distinction is also visible in the
[Open Rails descriptor reader](https://github.com/openrails/openrails/blob/master/Source/Orts.Formats.Msts/ShapeDescriptorFile.cs).
That reader uses text parsing; it is not an independent binary SD implementation
against which these binary tests were validated.
