# TSRE settings system — Phase 0 and Part 1
...
This implementation is the editable foundation for the new settings system. It deliberately does not replace values in `Game` or other runtime classes yet. That integration begins only after the provisional catalogue is reviewed at the start of Part 2.
...
During Part 1, `Game::load()` continues to read the existing legacy `settings.txt`
exactly as before. This preserves the application's current runtime behaviour until
the coordinated Phase 2B runtime cutover.
...
`profiles/default/settings.json`, while the legacy loader temporarily continues
to read existing values from `settings.txt` until the Phase 2B cutover.
...
Catalogue presence and runtime support are separate. The catalogue has completed
its Phase 2A approval, but Part 1 code still does not consume those values.
...
The editor's disabled support checkbox reflects these runtime claims. A claimed key with a different stored type is shown as a type mismatch. Part 2 will place support claims beside the code that actually consumes each setting.

Built-in definitions will also move from the provisional monolithic catalogue to
owner/module registration functions during Phase 2B. One central aggregation
function invokes every built-in registrar before profile loading, while the
central registry rejects duplicate or incompatible keys. This keeps definitions
extensible for forks without giving up deterministic startup or a common group
taxonomy.
...
The Phase 2A catalogue contains 76 profile settings: 74 migrated legacy
preferences plus the native startup-position switch and map API-key secret. The
disabled `gatherLegacyOverlays` renderer-test command and one-shot route merge
remain in the 76-key legacy audit but are not generated as profile settings. The
