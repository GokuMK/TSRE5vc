...
- Non-goal: per-file background refresh and per-file save/editor support (see `docs/tasks/addons/02-ref-refresh-and-per-file-management.md`).
...
- Ref parsing: `src/tsre/world/Ref.cpp:40` (`Ref::loadUtf16Data`)
- Template expansion: `src/tsre/world/Ref.cpp:193` (`Ref::expandTemplates`)
- Ref serialization (server + save): `src/tsre/world/Ref.cpp:291` (`Ref::saveToStream`)
...
### Includes inside `.ref`
The parser recognizes `Include ( "<file>" )` and uses `FileBuffer::insertFile(...)` to splice the included file into the current parse stream. `src/tsre/world/Ref.cpp:50`

Note: current include path handling concatenates `path + "/" + incPath`, where `path` is passed from `loadFile(...)` and is currently the *full path of the `.ref` file*. This assumes `path` is a directory, so include resolution may be incorrect unless call sites pass a directory string. `src/tsre/world/Ref.cpp:50`
...
- `template` (marks a template definition; expanded later into normal items)
- `unique` (template-only; when present, skips generating items whose filename already exists)
...
- non-template items are appended into `refItems[item.clas.trimmed()]`.
- template definitions are collected and expanded by `Ref::expandTemplates()` (called after addon loading).
...
See `Ref::RefItem::getNextShapeName()`. `src/tsre/world/Ref.cpp:341`
...
These pseudo-classes are inserted directly into `route->ref->refItems` and are intentionally not written back to disk. `src/routeEditor/ObjTools.cpp:243`, `src/tsre/world/Ref.cpp:296`
...
This serializer is also used by the multiplayer server to send addon data to remote clients (`request_addons`). `src/tsre/world/Ref.cpp:291`, `src/routeEditor/RouteEditorServer.cpp:304`

---

## 8. Known Limitations / Design Pressure Points

- **Templates expand only on load/manual reload**: there is no per-file background refresh and no per-file ownership tracking yet (see `docs/tasks/addons/02-ref-refresh-and-per-file-management.md`).
- **Directory is parsed but not applied for normal items** in placement (filenames must include the directory path themselves). For template items, `Directory` is used only for expansion time directory scanning.
