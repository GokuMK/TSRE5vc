...
namespace {
void readLayoutBlocks(FileBuffer &data, TerrainFileLegacy::LayoutInfo &info, int depth) {
    if (depth > 6)
...
bool TerrainFileLegacy::readLayoutInfo(const QString &path, LayoutInfo &info) {
    info = LayoutInfo{};
    QFile file(path);
...
TerrainFileLegacy::TerrainFileLegacy() {
    loaded = false;
    used = false;
}

TerrainFileLegacy::TerrainFileLegacy(const TerrainFileLegacy& orig) {
}

TerrainFileLegacy::~TerrainFileLegacy() {
}

float TerrainFileLegacy::patchValue(int patchId, PatchField field) const {
    return tdata[patchId * PatchFieldCount + static_cast<int>(field)];
}

void TerrainFileLegacy::setPatchValue(int patchId, PatchField field, float value) {
    tdata[patchId * PatchFieldCount + static_cast<int>(field)] = value;
}

void TerrainFileLegacy::initNew(QString name, int samples, int sampleS, int patches){
...
void TerrainFileLegacy::setBufferNames(QString name){
    sampleEbuffer = new QString(name + "_e.raw");
    sampleNbuffer = new QString(name + "_n.raw");
    sampleYbuffer = new QString(name + "_y.raw");
}

bool TerrainFileLegacy::readT(QString fSfile) {
...
bool TerrainFileLegacy::load(FileBuffer* data) {
    loaded = false;
    sampleMaterialBuffer.clear();
    bakedMaterialInfo.clear(); seasonalBakes.clear(); materialContentRevision=0; bakedMaterialsValid=true;
    ...
    try {
        ...
        while (data->off < end) {
            ...
            default: qDebug() << "TerrainFileLegacy - unknown token:" << TS::describe(block.id); break;
            }
            data->off = block.end;
        }
        loaded = true;
...
void TerrainFileLegacy::get139(FileBuffer* data, int length) {
        int slen;
        ...
        while (data->off < length) {
            ...
            switch (pozycja) {
                ...
                case TS::terrain_sample_usbuffer:
                    ...
                    break;
                default:
                    qDebug() << "TerrainFileLegacy - unknown token: "<< TS::describe(pozycja);
                    break;
            }
...
void TerrainFileLegacy::getOpaqueSampleBuffer(FileBuffer *data, int blockEnd,
...
int TerrainFileLegacy::opaqueSampleBufferBlockLength(const OpaqueSampleBuffer &buffer) {
    return 1 + buffer.label.length() * 2 + buffer.payload.size();
}

void TerrainFileLegacy::saveOpaqueSampleBuffer(QDataStream &write, TS::TokenId token,
...
void TerrainFileLegacy::get151(FileBuffer* data) {
    data->skipLabel();
...
void TerrainFileLegacy::get153(FileBuffer* data, TerrainFileLegacy::Mat* mat) {
    data->skipLabel();
    const int count = terrainBlockCount(data);
    if (count > 2)
...
void TerrainFileLegacy::get156(FileBuffer* data, TerrainFileLegacy::Mat* mat) {
    data->skipLabel();
    const int count = terrainBlockCount(data);
    if (count > 2)
...
void TerrainFileLegacy::get157(FileBuffer* data) {
    data->skipLabel();
...
void TerrainFileLegacy::get163(FileBuffer* data, int n) {
        data->skipLabel();
...
void TerrainFileLegacy::get251(FileBuffer* data) {
        data->skipLabel();
...
int TerrainFileLegacy::cloneMat(int id){
...
int TerrainFileLegacy::cloneAMat(int id){
    QString* name = new QString();
...
void TerrainFileLegacy::removeMat(int id){
...
int TerrainFileLegacy::getMatByTexture(QString tname){
...
bool TerrainFileLegacy::save(QString name){
...
void TerrainFileLegacy::save(QDataStream &write){
